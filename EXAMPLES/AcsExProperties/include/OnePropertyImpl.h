/* @(#) $Id: MonitoringTesterImpl.h,v 1.1 2007/06/06 23:04:30 rhiriart Exp $
 *
 * Copyright (C) 2001
 * Associated Universities, Inc. Washington DC, USA.
 *
 * Produced for the ALMA project
 *
 * This library is free software; you can redistribute it and/or modify it it 
 * under the terms of the GNU Library General Public License as published by 
 * the Free Software Foundation; either version 2 of the License, or (at your
 * option) any later version.
 *
 * This library is distributed in the hope that it will be useful but WITHOUT 
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or 
 * FITNESS FOR A PARTICULAR PURPOSE.  See the GNU Library General Public 
 * License for more details.
 *
 * You should have received a copy of the GNU Library General Public License 
 * along with this library; if not, write to the Free Software Foundation, 
 * Inc., 675 Massachusetts Ave, Cambridge, MA, 02139, USA.
 *
 * Correspondence concerning ALMA should be addressed as follows:
 * Internet email: alma-sw-admin@nrao.edu
 */
 
#ifndef OnePropertyImpl_h
#define OnePropertyImpl_h

#ifndef __cplusplus
#error This is a C++ include file and cannot be used from plain C
#endif

#include <baciCharacteristicComponentImpl.h>
#include <OnePropertyS.h>
#include <baciROdouble.h>
#include <baciRWdouble.h>
#include <baciSmartPropertyPointer.h>

using namespace baci;

class OnePropertyImpl : public CharacteristicComponentImpl,
			            public virtual POA_test::OneProperty
{
    public:

	OnePropertyImpl(const ACE_CString& name,
		            maci::ContainerServices* containerServices);

    virtual ~OnePropertyImpl();

    void initialize() throw(acsErrTypeLifeCycle::LifeCycleExImpl);

    virtual ACS::RWdouble_ptr doubleProp01() throw (CORBA::SystemException);

    private:

    SmartPropertyPointer<RWdouble> m_doubleProp01_sp;

    void operator=(const OnePropertyImpl&);

};

#endif /* OnePropertyImpl_h */
