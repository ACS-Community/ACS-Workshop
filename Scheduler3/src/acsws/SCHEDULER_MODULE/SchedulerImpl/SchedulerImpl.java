...
import cern.laser.source.alarmsysteminterface.*;
...
public class SchedulerImpl implements ComponentLifecycle, SchedulerOperations {
	...
	private String faultFamily="Scheduler";
	private  String faultMember="Scheduler3";
	private int faultCode1=100;
	private int faultCode2=200;
	private int faultCode3=300;
	...
	public void initialize(ContainerServices containerServices) {
		...
		try{
			...
			try {
				...
				FaultState fs = AlarmSystemInterfaceFactory.createFaultState(faultFamily, faultMember, faultCode1);
			...
			try{
				...
				AcsJContainerServicesEx ex3 = new AcsJContainerServicesEx(ex);
				FaultState fs = AlarmSystemInterfaceFactory.createFaultState(faultFamily, faultMember, faultCode2);
				fs.setDescriptor(ACSFaultState.ACTIVE);
				fs.setUserTimestamp(new Timestamp(System.currentTimeMillis()));
				alarmSource.push(fs);
				m_logger.warning(ex3.toString());
			...
			try{
				...
				AcsJContainerServicesEx ex3 = new AcsJContainerServicesEx(ex);
				FaultState fs = AlarmSystemInterfaceFactory.createFaultState(faultFamily, faultMember, faultCode3);
				fs.setDescriptor(ACSFaultState.ACTIVE);
				fs.setUserTimestamp(new Timestamp(System.currentTimeMillis()));
				alarmSource.push(fs);
				m_logger.warning(ex3.toString());
